...
class KafkaMessageProducer:
    ...
    async def send_message(self, topic: str, message: dict):
        ...
        Дополнительно выводит в лог поля name_uname и month_part сообщения.
        """
        ...
        try:
            ...
            self.logger.info(
                f"Message sent to topic {topic}. name_uname: {name_uname}, month_part: {month_part}."
            )
        except KafkaError as e:
            self.logger.error(f"Error sending message: {e}. Message: {message}")
            raise
