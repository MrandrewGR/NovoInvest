...
# Ключевые слова для определения релевантных таблиц
KEYWORDS = [
    "Реквизиты корпоративного действия",
    "Информация о ценных бумагах",
    "Детали корпоративного действия",
    "Информация о выплате дохода",
    # Добавьте дополнительные ключевые слова по мере необходимости
]
...
def parse_table(table):
    ...
    # Проверка, содержит ли заголовок ключевые слова
    ...
    if len(first_row_cells) == 2:
        ...
        if not headers:
            return None, None  # Пропускаем таблицу без заголовков

        data = []
        for row in rows:
            cells = row.find_all('td', recursive=False)
            if len(cells) != len(headers):
                continue  # Пропускаем строки с некорректным количеством столбцов
            row_data = {}
...
def parse_specific_tables(soup):
    """
    Парсит специфические таблицы на странице.
    """
    specific_tables = {}
    ...
    for table in tables:
        ...
        if any(keyword in title_text for keyword in KEYWORDS):
            _, data = parse_table(table)
            if data:
                specific_tables[title_text] = data
    return specific_tables


def parse_corporate_action_info(soup):
    """
    Извлекает заголовок, ИНН, ISIN и парсит специфические таблицы.
    """
    ...
    # Парсинг специфических таблиц
    specific_tables = parse_specific_tables(soup)
...
def main():
    ...
    for idx, u in enumerate(filtered_urls, start=1):
        ...
        # Введение увеличенной случайной задержки между 5 и 10 секундами
